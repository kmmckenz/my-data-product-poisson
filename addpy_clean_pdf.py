...
from Clean_Folder.Capstone_Milestone_3_Clean import load_data, log_step, log_content, about_data, scrub_data, outliers, scrub_new_data, about_cleaning
...
def clean():
    ...
    if scrubbed_data is None:
        return render_template_string("""
            ...
            <pre>{}</pre>
        """.format('\n'.join(log_content)))
    # save_cleaned_data(scrubbed_data, 'path/to/save/scrubbed_data.csv')
    # uncomment above to save the data
    ...
    if data_nomean_out is None:
        return render_template_string("""
            ...
            <pre>{}</pre>
        """.format('\n'.join(log_content)))
    # save_cleaned_data(data_nomean_out, 'path/to/save/data_nomean_out.csv')
    # uncomment above to save the cleaned data
    ...
    if data_nomedian_out is None:
        return render_template_string("""
            ...
            <pre>{}</pre>
        """.format('\n'.join(log_content)))
    # save_cleaned_data(data_nomedian_out, 'path/to/save/data_nomedian_out.csv')
    # uncomment above to save the cleaned data
    ...
    if data_log is None:
        return render_template_string("""
            ...
            <pre>{}</pre>
        """.format('\n'.join(log_content)))
    # save_cleaned_data(data_log, 'path/to/save/data_log.csv')
    # uncomment above to save the cleaned data
